...
func (m modeValue) Set(v string) error {
	for _, ok := range []string{"search", "tv-search", "movie-search", "music-search", "book-search"} {
		if v == ok {
			*m.target = v
			return nil
		}
...
func root() *cobra.Command {
	...
	searchCmd := &cobra.Command{Use: "search [query]", Args: cobra.MaximumNArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		c, err := config.Load("")
		if err != nil {
			return err
		}
		...
		r := search.Run(context.Background(), search.Options{Query: query, Mode: mode, Season: season, Episode: episode, IMDBID: imdbid, TMDBID: tmdbid, TVDBID: tvdbid, DoubanID: doubanid, TVMazeID: tvmazeid, Artist: artist, Album: album, Author: author, Title: title, Genre: genre, Year: year, Categories: categories, Indexers: idx, Limit: limit, Concurrency: conc, Debug: dbg})
		if out == "table" {
	...
	mode = "search"
	searchCmd.Flags().Var(modeValue{target: &mode}, "mode", strings.Join([]string{"search", "tv-search", "movie-search", "music-search", "book-search"}, "|"))
	...
	defs.AddCommand(&cobra.Command{Use: "sync", RunE: func(cmd *cobra.Command, args []string) error {
	...
	test := &cobra.Command{Use: "test [indexer-id]", Args: cobra.MaximumNArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		c, err := config.Load("")
		if err != nil {
			return err
		}
